...
<form action="" id="form-voucher">
	<div class="min-vh-100 d-flex flex-row align-items-center">
		...
		<a href="{{ route('dashboard_ticket') }}" class="btn btn-info position-absolute"
...
{{-- {{ route('redeem_voucher.cek_redeem_voucher') }} --}}
<script>
	setInterval(() => {
	...
	$('#form-voucher').on('submit', function(e) {
		...
		if (data.meta.code != 200) {
			...
			if (data.data.barcode_scan_status == 1019) {
				...
				Swal.fire({
					...
					html: `<p>${data.data.email}</p>
								...
								<p>E-Ticket Sudah di Redeem Pada ${data.data.barcode_scan_date}</p>
